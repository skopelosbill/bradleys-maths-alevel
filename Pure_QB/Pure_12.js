window.ALEVEL_QUESTIONS = [
 {
                ...
                "steps": [
                        "To find the vector $\\vec{AB}$, subtract the position vector of $A$ from $B$:\\begin{aligned} \\vec{AB} &= \\vec{OB} - \\vec{OA} \\cr &= \\begin{pmatrix} -2 \\cr -1 \\cr 3 \\end{pmatrix} - \\begin{pmatrix} 2 \\cr -3 \\cr 6 \\end{pmatrix} \\cr &= \\begin{pmatrix} -4 \\cr 2 \\cr -3 \\end{pmatrix} \\end{aligned}Calculate the exact length using the magnitude formula:\\begin{aligned} |\\vec{AB}| &= \\sqrt{(-4)^2 + 2^2 + (-3)^2} \\cr &= \\sqrt{16 + 4 + 9} \\cr &= \\sqrt{29} \\end{aligned}",
                        "The position vector of the midpoint $M$ is the average of $\\vec{OA}$ and $\\vec{OB}$:\\begin{aligned} \\vec{OM} &= \\dfrac{1}{2}(\\vec{OA} + \\vec{OB}) \\cr &= \\dfrac{1}{2}\\left[ \\begin{pmatrix} 2 \\cr -3 \\cr 6 \\end{pmatrix} + \\begin{pmatrix} -2 \\cr -1 \\cr 3 \\end{pmatrix} \\right] \\cr &= \\dfrac{1}{2}\\begin{pmatrix} 0 \\cr -4 \\cr 9 \\end{pmatrix} \\cr &= \\begin{pmatrix} 0 \\cr -2 \\cr \\dfrac{9}{2} \\end{pmatrix} \\end{aligned}",
                        "To prove that $ABPQ$ is a parallelogram, show that opposite side vectors are equal.<br><br>Find the first pair of opposite side vectors, $\\vec{AB}$ and $\\vec{QP}$:\\begin{aligned} \\vec{AB} &= \\begin{pmatrix} -4 \\cr 2 \\cr -3 \\end{pmatrix} \\cr \\vec{QP} &= \\vec{OP} - \\vec{OQ} \\cr &= \\begin{pmatrix} 2 \\cr 3 \\cr 1 \\end{pmatrix} - \\begin{pmatrix} 6 \\cr 1 \\cr 4 \\end{pmatrix} \\cr &= \\begin{pmatrix} -4 \\cr 2 \\cr -3 \\end{pmatrix} \\end{aligned}Since $\\vec{AB} = \\vec{QP}$, this pair of opposite sides is equal and parallel.<br><br>Now verify the second pair of opposite sides, $\\vec{BP}$ and $\\vec{AQ}$:\\begin{aligned} \\vec{BP} &= \\vec{OP} - \\vec{OB} \\cr &= \\begin{pmatrix} 2 \\cr 3 \\cr 1 \\end{pmatrix} - \\begin{pmatrix} -2 \\cr -1 \\cr 3 \\end{pmatrix} \\cr &= \\begin{pmatrix} 4 \\cr 4 \\cr -2 \\end{pmatrix} \\cr \\vec{AQ} &= \\vec{OQ} - \\vec{OA} \\cr &= \\begin{pmatrix} 6 \\cr 1 \\cr 4 \\end{pmatrix} - \\begin{pmatrix} 2 \\cr -3 \\cr 6 \\end{pmatrix} \\cr &= \\begin{pmatrix} 4 \\cr 4 \\cr -2 \\end{pmatrix} \\end{aligned}Since $\\vec{BP} = \\vec{AQ}$, the second pair is also equal and parallel. Therefore, $ABPQ$ is a parallelogram.",
                        "Final Answer: $$\\sqrt{29}$$"
                ],
                "pi_options": [
                        {
                                "ans": "$$\\sqrt{11}$$",
        ...
        },
        {
                ...
                "steps": [
                        "For $ABCD$ to be a parallelogram, the side vector $\\vec{AB}$ must equal the opposite side vector $\\vec{DC}$:\\begin{aligned} \\vec{AB} &= \\vec{DC} \\cr \\vec{OB} - \\vec{OA} &= \\vec{OC} - \\vec{OD} \\end{aligned}Rearrange to solve for the position vector $\\vec{OD}$:\\begin{aligned} \\vec{OD} &= \\vec{OA} + \\vec{OC} - \\vec{OB} \\cr &= \\begin{pmatrix} 1 \\cr 3 \\cr -2 \\end{pmatrix} + \\begin{pmatrix} 2 \\cr 5 \\cr 1 \\end{pmatrix} \\cr &\\qquad - \\begin{pmatrix} 4 \\cr -1 \\cr 3 \\end{pmatrix} \\cr &= \\begin{pmatrix} 1 + 2 - 4 \\cr 3 + 5 - (-1) \\cr -2 + 1 - 3 \\end{pmatrix} \\cr &= \\begin{pmatrix} -1 \\cr 9 \\cr -4 \\end{pmatrix} \\end{aligned}Thus, the coordinates of point $D$ are $(-1, 9, -4)$.",
                        "Find the displacement vector $\\vec{AC}$ by subtracting coordinates:\\begin{aligned} \\vec{AC} &= \\begin{pmatrix} 2 - 1 \\cr 5 - 3 \\cr 1 - (-2) \\end{pmatrix} \\cr &= \\begin{pmatrix} 1 \\cr 2 \\cr 3 \\end{pmatrix} \\end{aligned}Calculate the exact length using the 3D magnitude formula:\\begin{aligned} |\\vec{AC}| &= \\sqrt{1^2 + 2^2 + 3^2} \\cr &= \\sqrt{1 + 4 + 9} \\cr &= \\sqrt{14} \\end{aligned}",
                        "Two vectors are perpendicular if and only if their scalar dot product is zero. The diagonals are $\\vec{AC}$ and $\\vec{BD}$.<br><br>Find the vector of the second diagonal $\\vec{BD}$:\\begin{aligned} \\vec{BD} &= \\vec{OD} - \\vec{OB} \\cr &= \\begin{pmatrix} -1 \\cr 9 \\cr -4 \\end{pmatrix} - \\begin{pmatrix} 4 \\cr -1 \\cr 3 \\end{pmatrix} \\cr &= \\begin{pmatrix} -5 \\cr 10 \\cr -7 \\end{pmatrix} \\end{aligned}Compute the dot product $\\vec{AC} \\cdot \\vec{BD}$:\\begin{aligned} &\\vec{AC} \\cdot \\vec{BD} = \\begin{pmatrix} 1 \\cr 2 \\cr 3 \\end{pmatrix} \\cdot \\begin{pmatrix} -5 \\cr 10 \\cr -7 \\end{pmatrix} \\cr & \\quad = (1)(-5) + (2)(10) + (3)(-7) \\cr & \\quad= -5 + 20 - 21 \\cr & \\quad= -6 \\end{aligned}Since the scalar product is $-6 \\ne 0$, the diagonals are not perpendicular and do not intersect at right angles.",
                        "Final Answer: $$D(-1, 9, -4)$$"
                ],
                "pi_options": [
                        {
                                "ans": "$$D(1, 1, 6)$$",
        ...
        },
        {
                ...
                "steps": [
                        "Calculate the vectors $\\vec{AB}$ and $\\vec{BC}$ by subtracting coordinates:\\begin{aligned} \\vec{AB} &= \\vec{OB} - \\vec{OA} \\cr &= \\begin{pmatrix} 3 \\cr 2 \\cr 1 \\end{pmatrix} - \\begin{pmatrix} 1 \\cr 4 \\cr -2 \\end{pmatrix} \\cr &= \\begin{pmatrix} 2 \\cr -2 \\cr 3 \\end{pmatrix} \\end{aligned}\\begin{aligned} \\vec{BC} &= \\vec{OC} - \\vec{OB} \\cr &= \\begin{pmatrix} 7 \\cr -2 \\cr 7 \\end{pmatrix} - \\begin{pmatrix} 3 \\cr 2 \\cr 1 \\end{pmatrix} \\cr &= \\begin{pmatrix} 4 \\cr -4 \\cr 6 \\end{pmatrix} \\end{aligned}",
                        "Compare the vectors $\\vec{AB}$ and $\\vec{BC}$:\\begin{aligned} \\vec{BC} &= \\begin{pmatrix} 4 \\cr -4 \\cr 6 \\end{pmatrix} \\cr &= 2\\begin{pmatrix} 2 \\cr -2 \\cr 3 \\end{pmatrix} \\cr &= 2\\vec{AB} \\end{aligned}Since $\\vec{BC} = 2\\vec{AB}$, the vectors are parallel. Because they share the common point $B$, the points $A$, $B$, and $C$ lie on the same straight line and are collinear.",
                        "The point $D$ satisfies $\\vec{AD} = 2\\vec{DC}$. In terms of position vectors:\\begin{aligned} \\vec{OD} - \\vec{OA} &= 2(\\vec{OC} - \\vec{OD}) \\cr \\vec{OD} - \\vec{OA} &= 2\\vec{OC} - 2\\vec{OD} \\cr 3\\vec{OD} &= \\vec{OA} + 2\\vec{OC} \\cr \\vec{OD} &= \\dfrac{1}{3}(\\vec{OA} + 2\\vec{OC}) \\end{aligned}Substitute $\\vec{OA}$ and $\\vec{OC}$:\\begin{aligned} \\vec{OD} &= \\dfrac{1}{3}\\left[ \\begin{pmatrix} 1 \\cr 4 \\cr -2 \\end{pmatrix} + 2\\begin{pmatrix} 7 \\cr -2 \\cr 7 \\end{pmatrix} \\right] \\cr &= \\dfrac{1}{3}\\left[ \\begin{pmatrix} 1 \\cr 4 \\cr -2 \\end{pmatrix} + \\begin{pmatrix} 14 \\cr -4 \\cr 14 \\end{pmatrix} \\right] \\cr &= \\dfrac{1}{3}\\begin{pmatrix} 15 \\cr 0 \\cr 12 \\end{pmatrix} \\cr &= \\begin{pmatrix} 5 \\cr 0 \\cr 4 \\end{pmatrix} \\end{aligned}Thus, the coordinates of $D$ are $(5, 0, 4)$.",
                        "Final Answer: $$D(5, 0, 4)$$"
                ],
                "pi_options": [
                        {
                                "ans": "$$D(4, 1, 3)$$",
        ...
        },
        {
                ...
                "steps": [
                        "Find the displacement vectors $\\vec{PQ}$ and $\\vec{PR}$ by subtracting coordinates:\\begin{aligned} \\vec{PQ} &= \\vec{OQ} - \\vec{OP} \\cr &= \\begin{pmatrix} 4 \\cr y \\cr 1 \\end{pmatrix} - \\begin{pmatrix} 1 \\cr -2 \\cr 3 \\end{pmatrix} \\cr &= \\begin{pmatrix} 3 \\cr y + 2 \\cr -2 \\end{pmatrix} \\end{aligned}\\begin{aligned} \\vec{PR} &= \\vec{OR} - \\vec{OP} \\cr &= \\begin{pmatrix} 3 \\cr -3 \\cr 7 \\end{pmatrix} - \\begin{pmatrix} 1 \\cr -2 \\cr 3 \\end{pmatrix} \\cr &= \\begin{pmatrix} 2 \\cr -1 \\cr 4 \\end{pmatrix} \\end{aligned}",
                        "Since $PQ$ and $PR$ are perpendicular, their dot product equals zero:\\begin{aligned} &\\vec{PQ} \\cdot \\vec{PR} = 0 \\cr& \\quad \\begin{pmatrix} 3 \\cr y + 2 \\cr -2 \\end{pmatrix} \\cdot \\begin{pmatrix} 2 \\cr -1 \\cr 4 \\end{pmatrix} = 0 \\cr &\\quad (3)(2) + (y + 2)(-1)\\cr & \\qquad+ (-2)(4) = 0 \\cr & \\quad 6 - y - 2 - 8 = 0 \\cr & \\quad -y - 4 &= 0 \\cr y = -4 \\end{aligned}",
                        "With $y = -4$, the vector $\\vec{PQ}$ is:\\begin{aligned} \\vec{PQ} = \\begin{pmatrix} 3 \\cr -2 \\cr -2 \\end{pmatrix} \\end{aligned}The positive $z$-axis is represented by the unit vector $\\vec{k} = \\begin{pmatrix} 0 \\cr 0 \\cr 1 \\end{pmatrix}$. The cosine of the angle $\\theta$ is:\\begin{aligned} \\cos\\theta = \\dfrac{\\vec{PQ} \\cdot \\vec{k}}{|\\vec{PQ}| |\\vec{k}|} \\end{aligned}Find the length of $\\vec{PQ}$:\\begin{aligned} |\\vec{PQ}| &= \\sqrt{3^2 + (-2)^2 + (-2)^2} \\cr &= \\sqrt{9 + 4 + 4} \\cr &= \\sqrt{17} \\end{aligned}Compute the dot product and the cosine value:\\begin{aligned} \\vec{PQ} \\cdot \\vec{k} &= (3)(0) + (-2)(0) \\cr & \\qquad+ (-2)(1) \\cr &= -2 \\end{aligned}\\begin{aligned} \\cos\\theta &= \\dfrac{-2}{\\sqrt{17} \\times 1} \\cr &= -\\dfrac{2}{\\sqrt{17}} \\end{aligned}",
                        "Final Answer: $$-\\dfrac{2}{\\sqrt{17}}$$"
                ],
                "pi_options": [
                        {
                                "ans": "$$\\dfrac{3}{\\sqrt{17}}$$ ",
        ...
        },
        {
                ...
                "steps": [
                        "To show that triangle $ABC$ is equilateral, find the displacement vectors for all three sides:\\begin{aligned} \\vec{AB} &= \\begin{pmatrix} 3 - 2 \\cr -1 - 1 \\cr 2 - (-1) \\end{pmatrix} \\cr &= \\begin{pmatrix} 1 \\cr -2 \\cr 3 \\end{pmatrix} \\end{aligned}\\begin{aligned} \\vec{AC} &= \\begin{pmatrix} 5 - 2 \\cr 2 - 1 \\cr 1 - (-1) \\end{pmatrix} \\cr &= \\begin{pmatrix} 3 \\cr 1 \\cr 2 \\end{pmatrix} \\end{aligned}\\begin{aligned} \\vec{BC} &= \\begin{pmatrix} 5 - 3 \\cr 2 - (-1) \\cr 1 - 2 \\end{pmatrix} \\cr &= \\begin{pmatrix} 2 \\cr 3 \\cr -1 \\end{pmatrix} \\end{aligned}Calculate the length of each side vector:\\begin{aligned} |\\vec{AB}| &= \\sqrt{1^2 + (-2)^2 + 3^2} \\cr &= \\sqrt{14} \\cr |\\vec{AC}| &= \\sqrt{3^2 + 1^2 + 2^2} \\cr &= \\sqrt{14} \\cr |\\vec{BC}| &= \\sqrt{2^2 + 3^2 + (-1)^2} \\cr &= \\sqrt{14} \\end{aligned}Since $|\\vec{AB}| = |\\vec{AC}| = |\\vec{BC}| = \\sqrt{14}$, triangle $ABC$ is equilateral.",
                        "Find the scalar product $\\vec{AB} \\cdot \\vec{AC}$ using components:\\begin{aligned} \\vec{AB} \\cdot \\vec{AC} &= \\begin{pmatrix} 1 \\cr -2 \\cr 3 \\end{pmatrix} \\cdot \\begin{pmatrix} 3 \\cr 1 \\cr 2 \\end{pmatrix} \\cr &= (1)(3) + (-2)(1)\\cr & \\qquad + (3)(2) \\cr &= 3 - 2 + 6 \\cr &= 7 \\end{aligned}",
                        "Use the geometric definition of the scalar product to find $\\angle BAC$:\\begin{aligned} \\cos(\\angle BAC) &= \\dfrac{\\vec{AB} \\cdot \\vec{AC}}{|\\vec{AB}| |\\vec{AC}|} \\cr &= \\dfrac{7}{\\sqrt{14} \\times \\sqrt{14}} \\cr &= \\dfrac{7}{14} \\cr &= \\dfrac{1}{2} \\end{aligned}Since $\\cos(\\angle BAC) = \\dfrac{1}{2}$, the angle is:\\begin{aligned} \\angle BAC = 60^\\circ \\end{aligned}",
        ...
        },
{
                ...
                "steps": [
                        ...
                        "Final Answer: $$10 \\text{ hours per week}$$"
                ],
                "pi_options": [
                        {
        ...
        },
        {
                ...
                "steps": [
                        "In the linear cost model $C = mx + c$, when no units are produced ($x = 0$):\\begin{aligned} C &= m(0) + c \\cr &= c \\end{aligned}Therefore, the constant $c$ represents the fixed daily overhead costs of the business (such as rent, leasing, salaries, and utilities) that must be paid regardless of production output.",
                        "We are given $C = 1500$ when $x = 100$, and $C = 2700$ when $x = 250$. Substituting these gives a pair of equations:<br><br>Equation 1:\\begin{aligned} 100m + c = 1500 \\end{aligned}Equation 2:\\begin{aligned} 250m + c = 2700 \\end{aligned}Subtract Equation 1 from Equation 2:\\begin{aligned} 150m &= 1200 \\cr m &= 8 \\end{aligned}Substitute $m = 8$ back into Equation 1 to find $c$:\\begin{aligned} 100(8) + c &= 1500 \\cr 800 + c &= 1500 \\cr c &= 700 \\end{aligned}Thus, $m = 8$ and $c = 700$.",
        ...
        },
        {
                ...
                "steps": [
                        "The instant the projectile is launched corresponds to $t = 0$. Substituting $t = 0$ into our height equation:\\begin{aligned} h(0) &= 15 + 20(0) - 5(0)^2 \\cr &= 15 \\end{aligned}Thus, the height of the platform is $15$ metres.",
                        "To complete the square for $-5t^2 + 20t + 15$, factor out $-5$ from the variable terms:\\begin{aligned} h = -5(t^2 - 4t) + 15 \\end{aligned}Complete the square inside the bracket:\\begin{aligned} &h \\cr &\\quad = -5\\left[ (t - 2)^2 - 2^2 \\right] + 15 \\cr &\\quad = -5\\left[ (t - 2)^2 - 4 \\right] + 15 \\cr &\\quad = -5(t - 2)^2 + 20 + 15 \\cr &\\quad = 35 - 5(t - 2)^2 \\end{aligned}Since $-5(t - 2)^2 \\le 0$ for all real $t$, the maximum value of $h$ is $35$, which occurs when $t = 2$.<br><br>Therefore, the maximum height reached is $35$ metres, which occurs $2$ seconds after launch.",
                        "The projectile hits the ground when $h = 0$:\\begin{aligned} -5t^2 + 20t + 15 &= 0 \\cr t^2 - 4t - 3 &= 0 \\end{aligned}Solve using the quadratic formula:\\begin{aligned} &t \\cr &\\quad = \\dfrac{-(-4) \\pm \\sqrt{(-4)^2 - 4(1)(-3)}}{2(1)} \\cr &\\quad = \\dfrac{4 \\pm \\sqrt{16 + 12}}{2} \\cr &\\quad = \\dfrac{4 \\pm \\sqrt{28}}{2} \\cr &\\quad = \\dfrac{4 \\pm 2\\sqrt{7}}{2} \\cr &\\quad = 2 \\pm \\sqrt{7} \\end{aligned}Since time must be positive in this context, the impact time is:\\begin{aligned} t &= 2 + \\sqrt{7} \\cr &\\approx 4.65 \\text{ seconds} \\end{aligned}The model is valid from launch ($t = 0$) until ground impact ($t = 2 + \\sqrt{7}$):\\begin{aligned} 0 \\le t \\le 2 + \\sqrt{7} \\end{aligned}",
                        "Final Answer: $$\\text{Maximum height } 35\\text{ m at } t = 2\\text{ s}$$"
                ],
                "pi_options": [
                        {
        ...
        },
        {
                ...
                "img": false,
                "question": "<strong>(i)</strong> The temperature, $T$ ($^\\circ\\text{C}$), of a cup of hot coffee left in a room is modeled by the equation $T = 20 + 65\\mathrm{e}^{-0.05t}$, where $t$ is the time in minutes after the coffee is poured.<br><br><strong>(ii)</strong> Describe what happens to the temperature of the coffee as $t$ becomes very large ($t \\to \\infty$), and explain what this value represents in the context of the physical environment.<br><br><strong>(iii)</strong> Find the time, to the nearest minute, when the coffee's temperature reaches $45^\\circ\\text{C}$.",
                "steps": [
                        "The instant the coffee is poured corresponds to $t = 0$. Substituting $t = 0$ into our temperature model:\\begin{aligned} T(0) &= 20 + 65\\mathrm{e}^{-0.05(0)} \\cr &= 20 + 65(1) \\cr &= 85 \\end{aligned}Thus, the initial temperature of the coffee is $85^\\circ\\text{C}$.",
                        "To find what happens as $t \\to \\infty$, examine the exponential term:\\begin{aligned} \\lim_{t \\to \\infty} \\mathrm{e}^{-0.05t} = 0 \\end{aligned}Substituting this limiting behavior back into our equation:\\begin{aligned} T &\\to 20 + 65(0) \\cr &\\to 20 \\end{aligned}As $t$ becomes very large, the temperature decreases and approaches a steady limit of $20^\\circ\\text{C}$. In this physical context, this value represents the ambient temperature of the room.",
                        "Set the temperature $T$ to $45^\\circ\\text{C}$ and solve for $t$:\\begin{aligned} 45 &= 20 + 65\\mathrm{e}^{-0.05t} \\cr 25 &= 65\\mathrm{e}^{-0.05t} \\cr \\mathrm{e}^{-0.05t} &= \\dfrac{25}{65} \\cr &= \\dfrac{5}{13} \\end{aligned}Take the natural logarithm of both sides:\\begin{aligned} -0.05t &= \\ln\\left(\\dfrac{5}{13}\\right) \\cr t &= -\\dfrac{1}{0.05}\\ln\\left(\\dfrac{5}{13}\\right) \\cr &= -20\\ln\\left(\\dfrac{5}{13}\\right) \\end{aligned}Since $\\ln\\left(\\dfrac{5}{13}\\right) \\approx -0.9555$:\\begin{aligned} t &\\approx -20(-0.9555) \\cr &\\approx 19.11 \\text{ minutes} \\end{aligned}To the nearest minute, the coffee reaches $45^\\circ\\text{C}$ after $19$ minutes.",
                        "Final Answer: $$19 \\text{ minutes}$$"
                ],
                "pi_options": [
                        {
                                "ans": "$$26 \\text{ minutes}$$",
        ...
        },
       {
                ...
                "steps": [
                        ...
                        "For the piecewise function to be continuous at $d = 5$, the left-hand limit must equal the right-hand limit.<br><br>Evaluate the left-hand limit using the first branch:\\begin{aligned} \\lim_{d \\to 5^{-}} C(d) &= 45(5) \\cr &= 225 \\end{aligned}Evaluate the right-hand limit using the second branch:\\begin{aligned} \\lim_{d \\to 5^{+}} C(d) &= 225 + 30(5 - 5) \\cr &= 225 + 30(0) \\cr &= 225 \\end{aligned}Since both limits are equal to $225$, the two branches meet at the exact same point. Therefore, the function is continuous at $d = 5$.",
                        "Final Answer: $$C(4) = 180,\\ C(10) = 375$$"
                ],
                "pi_options": [
                        {
        ...
        },
{
                ...
                "img": false,
                "question": "<strong>(i)</strong> Prove that:\\begin{aligned} &2\\sin(\\theta + 45^\\circ)\\cos(\\theta + 45^\\circ) \\cr &\\qquad = \\cos 2\\theta \\end{aligned}",
                "steps": [
                        ...
                        "Apply the compound angle formula for sine:\\begin{aligned} \\sin(X + Y) &= \\sin X\\cos Y\\cr & \\qquad + \\cos X\\sin Y \\end{aligned}Expanding with $X = 2\\theta$ and $Y = 90^\\circ$:\\begin{aligned} &\\sin(2\\theta + 90^\\circ) \\cr &\\quad = \\sin 2\\theta\\cos 90^\\circ + \\cos 2\\theta\\sin 90^\\circ \\cr &\\quad = \\sin 2\\theta(0) + \\cos 2\\theta(1) \\cr &\\quad = \\cos 2\\theta \\end{aligned}Thus, the left-hand side simplifies to the right-hand side.",
        ...
        },
 {
                ...
                "steps": [
                        "To disprove the statement, find a counterexample where $n^2 + 3p = m^2$.<br><br>Choose the prime $p = 5$ and the square $n^2 = 49$ ($7^2$):\\begin{aligned} n^2 + 3p &= 49 + 3(5) \\cr &= 49 + 15 \\cr &= 64 \\end{aligned}Since $64 = 8^2$ is a square number, the statement is false.",
                        "Ben assumed that the only factors of $3p$ are $3$ and $p$.<br><br>Because $3$ and $p$ are both prime, the composite number $3p$ has four positive factors: $1$, $3$, $p$, and $3p$. Therefore, the factors $(m - n)$ and $(m + n)$ could also be $1$ and $3p$, which Ben failed to consider.<br><br>Furthermore, $2$ is the only even prime, so $p$ is odd for all other primes, making his parity claim invalid.",
                        "Set up the difference of two squares equation for $S = n^2$:\\begin{aligned} m^2 - n^2 &= 3(997) \\cr (m - n)(m + n) &= 2991 \\end{aligned}The positive factor pairs of $2991$ are $(3, 997)$ and $(1, 2991)$.<br><br>Case 1: Factor pair $(3, 997)$:\\begin{aligned} m - n &= 3 \\cr m + n &= 997 \\end{aligned}Subtracting gives:\\begin{aligned} 2n &= 994 \\cr n &= 497 \\end{aligned}This yields the square number:\\begin{aligned} S &= 497^2 \\cr &= 247009 \\end{aligned}Case 2: Factor pair $(1, 2991)$:\\begin{aligned} m - n &= 1 \\cr m + n &= 2991 \\end{aligned}Subtracting gives:\\begin{aligned} 2n &= 2990 \\cr n &= 1495 \\end{aligned}This yields $S = 1495^2 = 2235025$. Both values are valid square numbers.",
                        "Final Answer: $S = 247009$"
                ],
                "pi_options": [
                        {
                                "ans": "$S = 997^2$",
        ...
        },
        {
                ...
                "steps": [
                        "To disprove that $n^2 + n + 41$ is always prime, evaluate at $n = 41$:\\begin{aligned} n^2 + n + 41 &= 41^2 + 41 + 41 \\cr &= 41(41 + 1 + 1) \\cr &= 41(43) \\cr &= 1763 \\end{aligned}Since $1763 = 41 \\times 43$, it is a composite number. Thus, the claim is disproven by counterexample.",
                        "To disprove that the sum of two irrationals is always irrational, let $a = 3 + \\sqrt{2}$ and $b = 3 - \\sqrt{2}$. Both are irrational, but their sum is:\\begin{aligned} a + b &= (3 + \\sqrt{2}) + (3 - \\sqrt{2}) \\cr &= 6 \\end{aligned}Since $6$ is rational, this provides a valid counterexample.",
                        "Assume the negation: $n^2$ is even and $n$ is odd.<br><br>Let $n = 2k + 1$ for some integer $k$. Squaring both sides:\\begin{aligned} n^2 &= (2k + 1)^2 \\cr &= 4k^2 + 4k + 1 \\cr &= 2(2k^2 + 2k) + 1 \\cr &= 2M + 1 \\end{aligned}where $M = 2k^2 + 2k$ is an integer, meaning $n^2$ is odd.<br><br>This contradicts the premise that $n^2$ is even. Therefore, $n$ must be an even integer.",
                        "Final Answer: $n = 41 \\implies 1763$ which is composite"
                ],
                "pi_options": [
                        {
        ...
        },
        {
                ...
                "steps": [
                        "Test every integer in the interval $1 \\le n \\le 5$:<br><br>1. For $n = 1$: $1^3 - 1 = 0$, which is $3(0)$.<br><br>2. For $n = 2$: $2^3 - 2 = 6$, which is $3(2)$.<br><br>3. For $n = 3$: $3^3 - 3 = 24$, which is $3(8)$.<br><br>4. For $n = 4$: $4^3 - 4 = 60$, which is $3(20)$.<br><br>5. For $n = 5$: $5^3 - 5 = 120$, which is $3(40)$.<br><br>Since the statement holds true for all cases, the proof by exhaustion is complete.",
                        "Factor the expression completely:\\begin{aligned} n^3 - n &= n(n^2 - 1) \\cr &= n(n - 1)(n + 1) \\cr &= (n - 1)n(n + 1) \\end{aligned}This is the product of three consecutive integers.<br><br>In any three consecutive integers, at least one is a multiple of $2$ and exactly one is a multiple of $3$. Since $2$ and $3$ are coprime, the product is always a multiple of $2 \\times 3 = 6$.",
                        "Bound the expression between two consecutive squares for $n \\ge 1$:\\begin{aligned} (n + 1)^2 &= n^2 + 2n + 1 \\cr (n + 2)^2 &= n^2 + 4n + 4 \\end{aligned}For all positive integers $n$:\\begin{aligned} (n + 1)^2 < n^2 + 3n + 1 < (n + 2)^2 \\end{aligned}Since $n^2 + 3n + 1$ lies strictly between two consecutive perfect squares, it can never equal a perfect square.",
                        "Final Answer: $(n + 1)^2 < n^2 + 3n + 1 < (n + 2)^2$"
                ],
                "pi_options": [
                        {
                                "ans": "$n^3 - n = n^2(n - 1)$",
        ...
        },
        {
                ...
                "steps": [
                        "Let the consecutive square numbers be $n^2$ and $m^2$, where $m = n + 1$:\\begin{aligned} m^2 - n^2 &= p \\cr (m - n)(m + n) &= p \\end{aligned}Since $p$ is prime and $m > n$, its only positive integer factors are $1$ and $p$:\\begin{aligned} m - n &= 1 \\cr m + n &= p \\end{aligned}Adding gives $2m = p + 1$, so $m = \\dfrac{p + 1}{2}$.<br><br>Subtracting gives $2n = p - 1$, so $n = \\dfrac{p - 1}{2}$.<br><br>Since $p > 2$ is an odd prime, both $m$ and $n$ are integers:\\begin{aligned} p = \\left(\\dfrac{p + 1}{2}\\right)^2 - \\left(\\dfrac{p - 1}{2}\\right)^2 \\end{aligned}",
                        "Apply this result for $p = 101$:\\begin{aligned} m &= \\dfrac{101 + 1}{2} \\cr &= 51 \\cr n &= \\dfrac{101 - 1}{2} \\cr &= 50 \\end{aligned}Therefore:\\begin{aligned} 101 = 51^2 - 50^2 \\end{aligned}",
                        "Let the even square numbers be $m^2 = (2a)^2$ and $n^2 = (2b)^2$:\\begin{aligned} m^2 - n^2 &= 4a^2 - 4b^2 \\cr &= 4(a^2 - b^2) \\end{aligned}The difference of two even square numbers is always a multiple of $4$.<br><br>Since any prime $p > 2$ is odd, it cannot be a multiple of $4$. Therefore, no prime $p > 2$ can be written as the difference of two even square numbers.",
                        "Final Answer: $101 = 51^2 - 50^2$"
                ],
                "pi_options": [
                        {
                                "ans": "$101 = 52^2 - 49^2$",
        ...
        },
        {
                ...
                "steps": [
                        ...
                        "Assume the negation: $\\sqrt{2} = \\dfrac{a}{b}$ is rational in simplest form, where $a$ and $b$ are coprime integers and $b \\ne 0$.\\begin{aligned} 2 &= \\dfrac{a^2}{b^2} \\cr a^2 &= 2b^2 \\end{aligned}This implies $a^2$ is even, so $a$ is even. Let $a = 2k$:\\begin{aligned} (2k)^2 &= 2b^2 \\cr 4k^2 &= 2b^2 \\cr b^2 &= 2k^2 \\end{aligned}This implies $b^2$ is even, so $b$ is even. Both $a$ and $b$ share a common factor of $2$, which contradicts the assumption that $\\dfrac{a}{b}$ is in simplest form. Thus, $\\sqrt{2}$ is irrational.",
                        "Assume the negation: $3 + 2\\sqrt{2} = q$ for some rational number $q$.<br><br>Rearrange to isolate $\\sqrt{2}$:\\begin{aligned} 2\\sqrt{2} &= q - 3 \\cr \\sqrt{2} &= \\dfrac{q - 3}{2} \\end{aligned}Since $q$ is rational, $\\dfrac{q - 3}{2}$ must also be rational.<br><br>This contradicts part (ii), which established that $\\sqrt{2}$ is irrational. Therefore, $3 + 2\\sqrt{2}$ must be irrational.",
                        "Final Answer: $a^2 = 2b^2 \\implies a$ is even"
                ],
                "pi_options": [
                        {
        ...
        },
      {
                ...
                "steps": [
                        "The curve crosses the $x$-axis when $y = 0$:\\begin{aligned} \\dfrac{\\ln x}{x^2} &= 0 \\cr \\ln x &= 0 \\cr x &= \\mathrm{e}^0 \\cr x &= 1 \\end{aligned}Thus, the $x$-coordinate of the crossing point is $1$.",
                        "Show that the $y$-coordinates of $A$ and $B$ are equal.<br><br>For point $A$ ($x = \\sqrt{2}$):\\begin{aligned} y_A &= \\dfrac{\\ln(\\sqrt{2})}{(\\sqrt{2})^2} \\cr &= \\dfrac{\\ln(2^{1/2})}{2} \\cr &= \\dfrac{\\frac{1}{2}\\ln 2}{2} \\cr &= \\dfrac{\\ln 2}{4} \\end{aligned}For point $B$ ($x = 2$):\\begin{aligned} y_B &= \\dfrac{\\ln 2}{2^2} \\cr &= \\dfrac{\\ln 2}{4} \\end{aligned}Since $y_A = y_B = \\dfrac{\\ln 2}{4}$, the line $AB$ is horizontal and parallel to the $x$-axis.",
                        "Find the turning point using the quotient rule on $y = \\dfrac{\\ln x}{x^2}$ with $u = \\ln x$ and $v = x^2$:\\begin{aligned} &\\dfrac{\\text{d}y}{\\text{d}x} \\cr &\\quad = \\dfrac{u'v - uv'}{v^2} \\cr &\\quad = \\dfrac{\\left(\\frac{1}{x}\\right)(x^2) - (\\ln x)(2x)}{(x^2)^2} \\cr &\\quad = \\dfrac{x - 2x\\ln x}{x^4} \\cr &\\quad = \\dfrac{1 - 2\\ln x}{x^3} \\end{aligned}Set the derivative to zero:\\begin{aligned} 1 - 2\\ln x &= 0 \\cr \\ln x &= \\dfrac{1}{2} \\cr x &= \\mathrm{e}^{1/2} \\cr &= \\sqrt{\\mathrm{e}} \\end{aligned}Substitute $x = \\sqrt{\\mathrm{e}}$ to find the $y$-coordinate:\\begin{aligned} y &= \\dfrac{\\ln(\\sqrt{\\mathrm{e}})}{(\\sqrt{\\mathrm{e}})^2} \\cr &= \\dfrac{\\frac{1}{2}}{\\mathrm{e}} \\cr &= \\dfrac{1}{2\\mathrm{e}} \\end{aligned}Thus, the turning point is $\\left(\\sqrt{\\mathrm{e}}, \\dfrac{1}{2\\mathrm{e}}\\right)$.",
                        "Determine the nature of the turning point using the second derivative:\\begin{aligned} &\\dfrac{\\text{d}^2y}{\\text{d}x^2} \\cr &\\quad = \\dfrac{\\left(-\\frac{2}{x}\\right)(x^3) - (1 - 2\\ln x)(3x^2)}{(x^3)^2} \\cr &\\quad = \\dfrac{-2x^2 - 3x^2(1 - 2\\ln x)}{x^6} \\cr &\\quad = \\dfrac{-5 + 6\\ln x}{x^4} \\end{aligned}Evaluate at $x = \\sqrt{\\mathrm{e}}$ (where $\\ln x = \\dfrac{1}{2}$):\\begin{aligned} \\dfrac{\\text{d}^2y}{\\text{d}x^2} &= \\dfrac{-5 + 6\\left(\\frac{1}{2}\\right)}{(\\sqrt{\\mathrm{e}})^4} \\cr &= \\dfrac{-5 + 3}{\\mathrm{e}^2} \\cr &= -\\dfrac{2}{\\mathrm{e}^2} \\end{aligned}Since $-\\dfrac{2}{\\mathrm{e}^2} < 0$, the turning point is a local maximum.",
                        "Final Answer: $\\left(\\sqrt{\\mathrm{e}}, \\dfrac{1}{2\\mathrm{e}}\\right)$ is a local maximum"
                ],
                "pi_options": [
                        {
        ...
        },
        {
                ...
                "steps": [
                        "The curve crosses the $y$-axis when $x = 0$:\\begin{aligned} y(0) &= 2(0)^3 - 9(0)^2 + 12(0) - 3 \\cr &= -3 \\end{aligned}Thus, the coordinates of the crossing point are $(0, -3)$.",
                        "To find the turning points, set the first derivative to zero:\\begin{aligned} \\dfrac{\\text{d}y}{\\text{d}x} = 6x^2 - 18x + 12 \\end{aligned}\\begin{aligned} 6x^2 - 18x + 12 &= 0 \\cr 6(x^2 - 3x + 2) &= 0 \\cr 6(x - 1)(x - 2) &= 0 \\end{aligned}This gives $x = 1$ or $x = 2$.<br><br>1. For $x = 1$:\\begin{aligned} y &= 2(1)^3 - 9(1)^2 + 12(1) - 3 \\cr &= 2 - 9 + 12 - 3 \\cr &= 2 \\end{aligned}2. For $x = 2$:\\begin{aligned} y &= 2(2)^3 - 9(2)^2 + 12(2) - 3 \\cr &= 16 - 36 + 24 - 3 \\cr &= 1 \\end{aligned}Thus, the turning points are $(1, 2)$ and $(2, 1)$.",
                        "Find the nature using the second derivative:\\begin{aligned} \\dfrac{\\text{d}^2y}{\\text{d}x^2} = 12x - 18 \\end{aligned}1. At $x = 1$:\\begin{aligned} \\dfrac{\\text{d}^2y}{\\text{d}x^2} &= 12(1) - 18 \\cr &= -6 \\end{aligned}Since $-6 < 0$, the point $(1, 2)$ is a local maximum.<br><br>2. At $x = 2$:\\begin{aligned} \\dfrac{\\text{d}^2y}{\\text{d}x^2} &= 12(2) - 18 \\cr &= 6 \\end{aligned}Since $6 > 0$, the point $(2, 1)$ is a local minimum.",
                        "Find the gradient of the line connecting $(1, 2)$ and $(2, 1)$:\\begin{aligned} m &= \\dfrac{1 - 2}{2 - 1} \\cr &= \\dfrac{-1}{1} \\cr &= -1 \\end{aligned}",
                        "Final Answer: $(1, 2)$ and $(2, 1)$"
                ],
                "pi_options": [
                        {
                                "ans": "$(1, 2)$ and $(2, -3)$",
        ...
        },
        {
                ...
                "img": false,
                "question": "<strong>(i)</strong> Find the coordinates of the point where the curve $y = x^2 \\mathrm{e}^{-x}$ crosses the coordinate axes.<br><br><strong>(ii)</strong> Find the coordinates of the two stationary points on the curve, giving your answers in exact form.<br><br><strong>(iii)</strong> Use the second derivative to determine the nature of each stationary point.<br><br><strong>(iv)</strong> State what happens to the value of $y$ as $x \\to \\infty$.",
                "steps": [
                        "The curve crosses the $y$-axis when $x = 0$:\\begin{aligned} y(0) &= (0)^2 \\mathrm{e}^{-0} \\cr &= 0 \\end{aligned}Set $y = 0$ to find the $x$-intercepts:\\begin{aligned} x^2 \\mathrm{e}^{-x} = 0 \\end{aligned}Since $\\mathrm{e}^{-x} \\ne 0$, this requires $x = 0$. Thus, the curve crosses both axes at $(0, 0)$.",
                        "Use the product rule to find the first derivative, with $u = x^2$ and $v = \\mathrm{e}^{-x}$:\\begin{aligned} \\dfrac{\\text{d}y}{\\text{d}x} &= (2x)(\\mathrm{e}^{-x}) + (x^2)(-\\mathrm{e}^{-x}) \\cr &= (2x - x^2)\\mathrm{e}^{-x} \\cr &= x(2 - x)\\mathrm{e}^{-x} \\end{aligned}Set the gradient to zero:\\begin{aligned} x(2 - x)\\mathrm{e}^{-x} = 0 \\end{aligned}Since $\\mathrm{e}^{-x} \\ne 0$, this gives $x = 0$ or $x = 2$.<br><br>1. For $x = 0$: $y = 0 \\implies (0, 0)$.<br><br>2. For $x = 2$:\\begin{aligned} y &= 2^2 \\mathrm{e}^{-2} \\cr &= \\dfrac{4}{\\mathrm{e}^2} \\end{aligned}Thus, the stationary points are $(0, 0)$ and $\\left(2, \\dfrac{4}{\\mathrm{e}^2}\\right)$.",
                        "Differentiate $\\dfrac{\\text{d}y}{\\text{d}x} = (2x - x^2)\\mathrm{e}^{-x}$ using the product rule:\\begin{aligned} &\\dfrac{\\text{d}^2y}{\\text{d}x^2} \\cr &\\quad = (2 - 2x)\\mathrm{e}^{-x} + (2x - x^2)(-\\mathrm{e}^{-x}) \\cr &\\quad = (x^2 - 4x + 2)\\mathrm{e}^{-x} \\end{aligned}1. At $x = 0$:\\begin{aligned} \\dfrac{\\text{d}^2y}{\\text{d}x^2} &= (0 - 0 + 2)\\mathrm{e}^0 \\cr &= 2 \\end{aligned}Since $2 > 0$, $(0, 0)$ is a local minimum.<br><br>2. At $x = 2$:\\begin{aligned} \\dfrac{\\text{d}^2y}{\\text{d}x^2} &= (4 - 8 + 2)\\mathrm{e}^{-2} \\cr &= -\\dfrac{2}{\\mathrm{e}^2} \\end{aligned}Since $-\\dfrac{2}{\\mathrm{e}^2} < 0$, $\\left(2, \\dfrac{4}{\\mathrm{e}^2}\\right)$ is a local maximum.",
                        "Analyze the limiting behavior as $x \\to \\infty$:\\begin{aligned} \\lim_{x \\to \\infty} y &= \\lim_{x \\to \\infty} \\dfrac{x^2}{\\mathrm{e}^x} \\cr &= 0 \\end{aligned}As $x$ becomes large, $\\mathrm{e}^x$ dominates $x^2$, so $y \\to 0$.",
                        "Final Answer: $y \\to 0$ as $x \\to \\infty$"
                ],
                "pi_options": [
                        {
                ...
                "bradley_insight": {
                        ...
                        "content": "When differentiating functions multiplied by $\\mathrm{e}^{-x}$, the exponential term will always appear in every term of the derivative. To keep your work error-free, always factor out the $\\mathrm{e}^{-x}$ term immediately. This leaves a clean, simple polynomial that is easy to factorize and solve for stationary values."
                }
        },
        {
                "id": "004574",
        ...
        },
        {
                ...
                "img": false,
                "question": "<strong>(i)</strong> Find the $x$-coordinates of the points where the curve $y = \\mathrm{e}^x \\sin x$ crosses the $x$-axis on the interval $0 \\le x \\le \\pi$.<br><br><strong>(ii)</strong> Find the exact coordinates of the stationary point on the curve within this interval.<br><br><strong>(iii)</strong> Show that this stationary point is a local maximum.",
                "steps": [
