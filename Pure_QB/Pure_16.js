...
},
{
    ...
    "steps": [
        ...
        "To determine if it is an overestimate or underestimate, analyze the curvature (concavity) of $y = \\mathrm{e}^{ -x }$ by finding its second derivative:\\begin{aligned} &y' = -\\mathrm{e}^{ -x } \\cr &y'' = \\mathrm{e}^{ -x } \\end{aligned}Since $\\mathrm{e}^{ -x } > 0$ for all real values of $x$, we have $y'' > 0$ on the interval $[0, 2]$, which means the curve is concave up (convex).<br><br>Because the curve is concave up, the straight-line chords forming the tops of the trapeziums lie entirely above the actual curve. Therefore, the approximation is an **overestimate**.",
