window.ALEVEL_QUESTIONS = [
{
    ...
    "steps": [
        "<strong>(a) Find the acceleration vector:</strong><br><br>The weight acts vertically downwards ($W = mg$):\\begin{aligned} &\\mathbf{W} \\cr &\\quad = -5(9.8)\\mathbf{j} \\cr &\\quad = -49\\mathbf{j}\\text{ N} \\end{aligned}Sum all forces acting on $P$:\\begin{aligned} &\\Sigma\\mathbf{F} \\cr &\\quad = \\mathbf{F}_1 + \\mathbf{F}_2 + \\mathbf{W} \\cr &\\quad = (18 - 8)\\mathbf{i} \\cr &\\qquad + (-6 + 10 - 49)\\mathbf{j} \\cr &\\quad = (10\\mathbf{i} - 45\\mathbf{j})\\text{ N} \\end{aligned}Using $\\mathbf{F} = m\\mathbf{a}$ with $m = 5\\text{ kg}$:\\begin{aligned} &\\mathbf{a} \\cr &\\quad = \\dfrac{10\\mathbf{i} - 45\\mathbf{j}}{5} \\cr &\\quad = (2\\mathbf{i} - 9\\mathbf{j})\\text{ m s}^{-2} \\end{aligned}",
        "<strong>(b) Find position vector at $t = 6\\text{ s}$:</strong><br><br>Using $\\mathbf{r} = \\mathbf{r}_0 + \\mathbf{u}t + \\dfrac{1}{2}\\mathbf{a}t^2$ with $\\mathbf{u} = \\mathbf{0}$:\\begin{aligned} &\\mathbf{r}(6) \\cr &\\quad = (3\\mathbf{i} + 40\\mathbf{j}) \\cr &\\qquad + \\dfrac{1}{2}(2\\mathbf{i} - 9\\mathbf{j})(6^2) \\cr &\\quad = (3\\mathbf{i} + 40\\mathbf{j}) \\cr &\\qquad + 18(2\\mathbf{i} - 9\\mathbf{j}) \\cr &\\quad = (3 + 36)\\mathbf{i} \\cr &\\qquad + (40 - 162)\\mathbf{j} \\cr &\\quad = (39\\mathbf{i} - 122\\mathbf{j})\\text{ m} \\end{aligned}",
        "<strong>(c) Find speed at horizontal level of the origin:</strong><br><br>The particle reaches the level of the origin when the vertical displacement is zero ($y = 0$):\\begin{aligned} &40 - \\dfrac{1}{2}(9)t^2 = 0 \\cr &4.5t^2 = 40 \\cr &t^2 \\cr &\\quad = \\dfrac{40}{4.5} \\cr &\\quad = \\dfrac{80}{9} \\cr &t \\cr &\\quad = \\dfrac{\\sqrt{80}}{3}\\text{ s} \\end{aligned}The velocity vector at this time is $\\mathbf{v} = \\mathbf{a}t$:\\begin{aligned} &\\mathbf{v} \\cr &\\quad = t(2\\mathbf{i} - 9\\mathbf{j}) \\end{aligned}Calculate speed $|\\mathbf{v}|$:\\begin{aligned} &|\\mathbf{v}| \\cr &\\quad = t\\sqrt{2^2 + (-9)^2} \\cr &\\quad = \\dfrac{\\sqrt{80}}{3}\\sqrt{85} \\cr &\\quad = \\dfrac{\\sqrt{6800}}{3} \\cr &\\quad = \\dfrac{20\\sqrt{17}}{3} \\cr &\\quad \\approx 27.5\\text{ m s}^{-1} \\end{aligned}",
...
},
{
    ...
    "steps": [
        "<strong>(a) Show acceleration is $(3\\mathbf{i} - 5\\mathbf{j})\\text{ m s}^{-2}$:</strong><br><br>The forces acting on the drone are thrust $\\mathbf{F}$ and weight $\\mathbf{W} = -mg\\mathbf{j}$:\\begin{aligned} &\\mathbf{W} \\cr &\\quad = -2(9.8)\\mathbf{j} \\cr &\\quad = -19.6\\mathbf{j}\\text{ N} \\end{aligned}Sum of forces:\\begin{aligned} &\\Sigma\\mathbf{F} \\cr &\\quad = (6\\mathbf{i} + 9.6\\mathbf{j}) - 19.6\\mathbf{j} \\cr &\\quad = (6\\mathbf{i} - 10\\mathbf{j})\\text{ N} \\end{aligned}Using $\\mathbf{a} = \\dfrac{\\Sigma\\mathbf{F}}{m}$ with $m = 2\\text{ kg}$:\\begin{aligned} &\\mathbf{a} \\cr &\\quad = \\dfrac{6\\mathbf{i} - 10\\mathbf{j}}{2} \\cr &\\quad = (3\\mathbf{i} - 5\\mathbf{j})\\text{ m s}^{-2} \\end{aligned}",
        "<strong>(b) Find time $T$ when travelling horizontally:</strong><br><br>Travelling horizontally means the vertical component of velocity is zero ($v_y = 0$).<br><br>Using $v_y = u_y + a_y t$:\\begin{aligned} &25 - 5T = 0 \\cr &5T = 25 \\cr &T \\cr &\\quad = \\dfrac{25}{5} \\cr &\\quad = 5\\text{ s} \\end{aligned}",
        "<strong>(c) Find position vector at $t = 5\\text{ s}$:</strong><br><br>Using $\\mathbf{r} = \\mathbf{u}t + \\dfrac{1}{2}\\mathbf{a}t^2$ with $t = 5\\text{ s}$:\\begin{aligned} &\\mathbf{r}(5) \\cr &\\quad = (4\\mathbf{i} + 25\\mathbf{j})(5) \\cr &\\qquad + \\dfrac{1}{2}(3\\mathbf{i} - 5\\mathbf{j})(5^2) \\cr &\\quad = (20\\mathbf{i} + 125\\mathbf{j}) \\cr &\\qquad + 12.5(3\\mathbf{i} - 5\\mathbf{j}) \\cr &\\quad = (20 + 37.5)\\mathbf{i} \\cr &\\qquad + (125 - 62.5)\\mathbf{j} \\cr &\\quad = (57.5\\mathbf{i} + 62.5\\mathbf{j})\\text{ m} \\end{aligned}",
        "<strong>(d) Calculate bearing at $t = 7\\text{ s}$:</strong><br><br>Find the velocity vector at $t = 7\\text{ s}$ using $\\mathbf{v} = \\mathbf{u} + \\mathbf{a}t$:\\begin{aligned} &\\mathbf{v}(7) \\cr &\\quad = (4\\mathbf{i} + 25\\mathbf{j}) + 7(3\\mathbf{i} - 5\\mathbf{j}) \\cr &\\quad = (4 + 21)\\mathbf{i} \\cr &\\qquad + (25 - 35)\\mathbf{j} \\cr &\\quad = (25\\mathbf{i} - 10\\mathbf{j})\\text{ m s}^{-1} \\end{aligned}The velocity vector has positive East component ($25$) and negative North component (South component $10$).<br><br>Find angle $\\alpha$ East of South:\\begin{aligned} &\\tan\\alpha \\cr &\\quad = \\dfrac{25}{10} \\cr &\\quad = 2.5 \\cr &\\alpha \\cr &\\quad = \\arctan(2.5) \\cr &\\quad \\approx 68.2^{\\circ} \\end{aligned}Calculate bearing clockwise from North:\\begin{aligned} &\\text{Bearing} \\cr &\\quad = 180^{\\circ} - 68.2^{\\circ} \\cr &\\quad = 111.8^{\\circ} \\cr &\\quad \\approx 112^{\\circ} \\end{aligned}",
...
},
{
    ...
    "steps": [
        "<strong>(a) Find the resultant force vector:</strong><br><br>Find the unit vector in the direction of $3\\mathbf{i} + 4\\mathbf{j}$:\\begin{aligned} &|3\\mathbf{i} + 4\\mathbf{j}| \\cr &\\quad = \\sqrt{3^2 + 4^2} \\cr &\\quad = 5 \\cr &\\hat{\\mathbf{a}} \\cr &\\quad = \\dfrac{3\\mathbf{i} + 4\\mathbf{j}}{5} \\cr &\\quad = 0.6\\mathbf{i} + 0.8\\mathbf{j} \\end{aligned}Acceleration vector has magnitude $2.5\\text{ m s}^{-2}$:\\begin{aligned} &\\mathbf{a} \\cr &\\quad = 2.5(0.6\\mathbf{i} + 0.8\\mathbf{j}) \\cr &\\quad = (1.5\\mathbf{i} + 2\\mathbf{j})\\text{ m s}^{-2} \\end{aligned}Resultant force $\\mathbf{R} = m\\mathbf{a}$ with $m = 8\\text{ kg}$:\\begin{aligned} &\\mathbf{R} \\cr &\\quad = 8(1.5\\mathbf{i} + 2\\mathbf{j}) \\cr &\\quad = (12\\mathbf{i} + 16\\mathbf{j})\\text{ N} \\end{aligned}",
        "<strong>(b) Determine the values of $p$ and $q$:</strong><br><br>Sum the three applied forces:\\begin{aligned} &\\Sigma\\mathbf{F} \\cr &\\quad = \\mathbf{F}_1 + \\mathbf{F}_2 + \\mathbf{F}_3 \\cr &\\quad = (p + 6 - 2)\\mathbf{i} \\cr &\\qquad + (14 + q - 5)\\mathbf{j} \\cr &\\quad = (p + 4)\\mathbf{i} + (q + 9)\\mathbf{j} \\end{aligned}Equate components to resultant $\\mathbf{R} = 12\\mathbf{i} + 16\\mathbf{j}$:\\begin{aligned} &p + 4 = 12 \\cr &p = 8 \\end{aligned}For vertical components:\\begin{aligned} &q + 9 = 16 \\cr &q = 7 \\end{aligned}",
        "<strong>(c) Find the distance travelled in $6\\text{ s}$:</strong><br><br>Since the sledge accelerates from rest ($u = 0$) with constant acceleration magnitude $a = 2.5\\text{ m s}^{-2}$:\\begin{aligned} &s \\cr &\\quad = ut + \\dfrac{1}{2}at^2 \\cr &\\quad = 0 + \\dfrac{1}{2}(2.5)(6^2) \\cr &\\quad = 1.25 \\times 36 \\cr &\\quad = 45\\text{ m} \\end{aligned}",
...
},
{
    ...
    "steps": [
        "<strong>(a) Find the acceleration vector:</strong><br><br>Using Newton's second law $\\mathbf{a} = \\dfrac{\\mathbf{F}}{m}$ with $m = 0.5\\text{ kg}$:\\begin{aligned} &\\mathbf{a}(t) \\cr &\\quad = \\dfrac{(6t - 4)\\mathbf{i} + (12 - 9t^2)\\mathbf{j}}{0.5} \\cr &\\quad = (12t - 8)\\mathbf{i} \\cr &\\qquad + (24 - 18t^2)\\mathbf{j}\\text{ m s}^{-2} \\end{aligned}",
        "<strong>(b) Find the velocity vector $\\mathbf{v}(t)$:</strong><br><br>Integrate acceleration with respect to time $t$:\\begin{aligned} &\\mathbf{v}(t) \\cr &\\quad = \\int \\mathbf{a}\\text{d}t \\cr &\\quad = (6t^2 - 8t)\\mathbf{i} \\cr &\\qquad + (24t - 6t^3)\\mathbf{j} + \\mathbf{C} \\end{aligned}Since $\\mathbf{v}(0) = 2\\mathbf{i} - 6\\mathbf{j}$, we have $\\mathbf{C} = 2\\mathbf{i} - 6\\mathbf{j}$:\\begin{aligned} &\\mathbf{v}(t) \\cr &\\quad = (6t^2 - 8t + 2)\\mathbf{i} \\cr &\\qquad + (24t - 6t^3 - 6)\\mathbf{j}\\text{ m s}^{-1} \\end{aligned}",
        "<strong>(c) Find kinetic energy at $t = 2\\text{ s}$:</strong><br><br>Evaluate velocity at $t = 2\\text{ s}$:\\begin{aligned} &\\mathbf{v}(2) \\cr &\\quad = (6(2^2) - 8(2) + 2)\\mathbf{i} \\cr &\\qquad + (24(2) - 6(2^3) - 6)\\mathbf{j} \\cr &\\quad = (24 - 16 + 2)\\mathbf{i} \\cr &\\qquad + (48 - 48 - 6)\\mathbf{j} \\cr &\\quad = (10\\mathbf{i} - 6\\mathbf{j})\\text{ m s}^{-1} \\end{aligned}Calculate speed squared $v^2$:\\begin{aligned} &v^2 \\cr &\\quad = 10^2 + (-6)^2 \\cr &\\quad = 100 + 36 \\cr &\\quad = 136 \\end{aligned}Calculate kinetic energy ($\\text{KE} = \\dfrac{1}{2}mv^2$):\\begin{aligned} &\\text{KE} \\cr &\\quad = \\dfrac{1}{2}(0.5)(136) \\cr &\\quad = 34\\text{ J} \\end{aligned}",
        "<strong>(d) Find position vector at $t = 3\\text{ s}$:</strong><br><br>Integrate velocity to find position $\\mathbf{r}(t)$:\\begin{aligned} &\\mathbf{r}(t) \\cr &\\quad = \\int \\mathbf{v}\\text{d}t \\cr &\\quad = (2t^3 - 4t^2 + 2t)\\mathbf{i} \\cr &\\qquad + (12t^2 - 1.5t^4 - 6t)\\mathbf{j} + \\mathbf{D} \\end{aligned}Since $P$ starts at the origin $O$, $\\mathbf{r}(0) = \\mathbf{0}$, giving $\\mathbf{D} = \\mathbf{0}$.<br><br>At $t = 3\\text{ s}$:\\begin{aligned} &\\mathbf{r}(3) \\cr &\\quad = (2(27) - 4(9) + 2(3))\\mathbf{i} \\cr &\\qquad + (12(9) - 1.5(81) - 6(3))\\mathbf{j} \\cr &\\quad = (54 - 36 + 6)\\mathbf{i} \\cr &\\qquad + (108 - 121.5 - 18)\\mathbf{j} \\cr &\\quad = (24\\mathbf{i} - 31.5\\mathbf{j})\\text{ m} \\end{aligned}",
...
},
{
    ...
    "steps": [
        "<strong>(a) Show acceleration of $B$ is $(2\\mathbf{i} + 2\\mathbf{j})\\text{ m s}^{-2}$:</strong><br><br>Using $\\mathbf{a} = \\dfrac{\\mathbf{F}}{m}$ with $\\mathbf{F} = 6\\mathbf{i} + 6\\mathbf{j}$ and $m = 3\\text{ kg}$:\\begin{aligned} &\\mathbf{a}_B \\cr &\\quad = \\dfrac{6\\mathbf{i} + 6\\mathbf{j}}{3} \\cr &\\quad = (2\\mathbf{i} + 2\\mathbf{j})\\text{ m s}^{-2} \\end{aligned}",
        "<strong>(b) Find position vectors $\\mathbf{r}_A$ and $\\mathbf{r}_B$:</strong><br><br>Car $A$ moves with constant velocity from $(15\\mathbf{i} + 5\\mathbf{j})\\text{ m}$:\\begin{aligned} &\\mathbf{r}_A \\cr &\\quad = (15\\mathbf{i} + 5\\mathbf{j}) + t(2\\mathbf{i} + 4\\mathbf{j}) \\cr &\\quad = (15 + 2t)\\mathbf{i} \\cr &\\qquad + (5 + 4t)\\mathbf{j} \\end{aligned}Car $B$ starts from rest at the origin with constant acceleration:\\begin{aligned} &\\mathbf{r}_B \\cr &\\quad = \\mathbf{0} + \\dfrac{1}{2}(2\\mathbf{i} + 2\\mathbf{j})t^2 \\cr &\\quad = t^2\\mathbf{i} + t^2\\mathbf{j} \\end{aligned}",
        "<strong>(c) Show displacement vector $\\mathbf{r}_{BA}$:</strong><br><br>Displacement from $B$ to $A$ is $\\mathbf{r}_A - \\mathbf{r}_B$:\\begin{aligned} &\\mathbf{r}_{BA} \\cr &\\quad = \\mathbf{r}_A - \\mathbf{r}_B \\cr &\\quad = \\big[(15 + 2t)\\mathbf{i} + (5 + 4t)\\mathbf{j}\\big] \\cr &\\qquad - (t^2\\mathbf{i} + t^2\\mathbf{j}) \\cr &\\quad = (15 + 2t - t^2)\\mathbf{i} \\cr &\\qquad + (5 + 4t - t^2)\\mathbf{j} \\end{aligned}",
        "<strong>(d) Show cars collide and find collision coordinates:</strong><br><br>A collision occurs when $\\mathbf{r}_{BA} = \\mathbf{0}$, meaning both components are simultaneously zero.<br><br>For the horizontal component:\\begin{aligned} &15 + 2t - t^2 = 0 \\cr &t^2 - 2t - 15 = 0 \\cr &(t - 5)(t + 3) = 0 \\end{aligned}Since $t \\ge 0$, the valid root is $t = 5\\text{ s}$.<br><br>For the vertical component:\\begin{aligned} &5 + 4t - t^2 = 0 \\cr &t^2 - 4t - 5 = 0 \\cr &(t - 5)(t + 1) = 0 \\end{aligned}Since $t \\ge 0$, the valid root is $t = 5\\text{ s}$.<br><br>Since $t = 5\\text{ s}$ satisfies both equations simultaneously, the cars collide at $t = 5\\text{ s}$.<br><br>Find collision coordinates at $t = 5\\text{ s}$:\\begin{aligned} &\\mathbf{r}(5) \\cr &\\quad = 5^2\\mathbf{i} + 5^2\\mathbf{j} \\cr &\\quad = (25\\mathbf{i} + 25\\mathbf{j})\\text{ m} \\end{aligned}The coordinates of the collision point are $(25, 25)$.",
